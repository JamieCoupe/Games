...
# This is the sonar treasure hunt game created by
...
def enter_player_move(previous_moves):
    ...
    print('Where do you want to drop the next sonar device? (0-59 0-14) (or type "q" to quit)')
    while True:
        move = input()
        if move.lower() == 'q':
...
def play_game():
    ...
    while True:
        ...
        while sonarDevices > 0:
            ...
            print('You have {} sonar device(s) left. {} treasure chest(s) remaining.'.format(sonarDevices,
                                                                                             len(theChests)))
            ...
            move_result = make_move(theBoard, theChests, x, y)
            if not move_result:
                continue
            else:
